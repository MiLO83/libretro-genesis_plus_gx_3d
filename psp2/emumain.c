...
void InitEmulator()
{
  ...
  memset(&bitmap_L, 0, sizeof(t_bitmap));
  memset(&bitmap_R, 0, sizeof(t_bitmap));
  memset(&bitmap_LR, 0, sizeof(t_bitmap*2));
  bitmap.width  = Screen->Width/2;
  bitmap.height = Screen->Height;
  bitmap.pitch  = (bitmap.width * 2);
  bitmap.data   = (unsigned char *)Screen->Pixels;
  bitmap_L.width  = Screen->Width/2;
  bitmap_L.height = Screen->Height;
  bitmap_L.pitch  = (bitmap.width * 2);
  bitmap_L.data   = (unsigned char *)Screen->Pixels;
  bitmap_R.width  = Screen->Width/2;
  bitmap_R.height = Screen->Height;
  bitmap_R.pitch  = (bitmap.width * 2);
  bitmap_R.data   = (unsigned char *)Screen->Pixels;
  bitmap_LR.width  = Screen->Width;
  bitmap_LR.height = Screen->Height;
  bitmap_LR.pitch  = (bitmap.width * 2) * 2;
  bitmap_LR.data   = (unsigned char *)Screen->Pixels;
...
//pl_rewind_realloc(&Rewinder);
  ...
  while (!ExitPSP&&bEmulate)
  {
    ...
    if (++Frame <= Options.Frameskip)
    {
      ...
      {
          system_frame_gen_left(1);
          memcpy(bitmap_data_L_, bitmap_data_, (sizeof(bitmap_data_)));
          system_frame_gen_right(1);
          memcpy(bitmap_data_R_, bitmap_data_, (sizeof(bitmap_data_)));
      }
      else
      {
        system_frame_sms(1);
    ...
    else
    {
      ...
      {
          system_frame_gen_left(0);
          memcpy(bitmap_data_L_, bitmap_data_, (sizeof(bitmap_data_)));
          system_frame_gen_right(0);
          memcpy(bitmap_data_R_, bitmap_data_, (sizeof(bitmap_data_)));
      }
      else
      {
